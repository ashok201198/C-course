...
char * subString(char *str, int a, int b){
	...
	char * ret = (char *)malloc(sizeof(char)*(b - a+10));
	for (i = a-1; i < b; i++)
	{
		ret[k] = str[i];
		k++;
...
char * invokeSubStr(char * (*function_ptr)(char *, int, int), char * str, int a, int b) {
	...
	//char *c = (char *)malloc(sizeof(char)*(b - a + 2));
	char *c;
	c=function_ptr(str, a, b);
	return c;
...
void task3() {
	...
	char* (*p)(char *, int, int)=subString;
	//char *c=(char *)malloc(sizeof(char)*(100));
	char *c;
	c=invokeSubStr(p, "ashok", 1, 2);
}
