...
void typeCastToAllDataTypes(void * long_ptr, void * float_ptr, void * char_ptr, void * str_ptr) {
	...
	printf("Value stored at char_ptr is %s\n", *(char *)str_ptr);
	//Other code
}
...
void task6() {
	...
	typeCastToAllDataTypes((void *)a,p, (void *)c, (void *)d);
}
...
void ** getTwoGreater(void *addr1, void *addr2, void *addr3, int(*compare)(void *, void *)) {
	...
	int a = compare(addr1, addr2);
	int c;
	int b;
	if (a == 1||a==0)
	{
		b = compare(addr2, addr3);
		if (b == 1||b==0)
		{
			result[0] = addr1;
			result[1] = addr2;
